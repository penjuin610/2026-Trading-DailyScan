...
def compute_breadth(
    ...
    pct_green_min = cfg.get("pct_above_200ema_green", 0.55)
    pct_yellow_min = cfg.get("pct_above_200ema_yellow", 0.40)
    adv_green_min = cfg.get("adv_ratio_min", 0.50)
    adv_yellow_min = cfg.get("adv_ratio_yellow_min", max(0.35, adv_green_min - 0.15))
    nh_nl_green_min = cfg.get("nh_nl_ratio_min", 1.2)
    nh_nl_yellow_min = cfg.get("nh_nl_ratio_yellow_min", max(0.8, nh_nl_green_min - 0.4))
    ...
    green_ok = (
        score >= green_th and
        pct_above_200 >= pct_green_min and
        adv_ratio >= adv_green_min and
        nh_nl_ratio >= nh_nl_green_min
    )
    yellow_ok = (
        score >= yellow_th and
        pct_above_200 >= pct_yellow_min and
        adv_ratio >= adv_yellow_min and
        nh_nl_ratio >= nh_nl_yellow_min
    )

    level = "HEALTHY" if green_ok else ("MIXED" if yellow_ok else "WEAK")
    ...
    if adv_ratio < adv_green_min:
        notes.append(f"Advance/decline weak ({adv_ratio*100:.0f}% advancers)")
    if leading:
